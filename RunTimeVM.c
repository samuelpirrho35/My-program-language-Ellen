...
**      {
                   //  FIELD ONE                       FIELD TWO
            INIT_SET, STORAGE_I32, END_SET, .... INIT_SET, SEND, I8, 0X00,
            END_SET0, .... INIT_SET, SEND, I32, 0X00, 0X00, 0X00, 0X00, END_SET
                                   //   FIELD THREE
        }
*/
        ...
        while(line[++(*addr_i)] != END_SET){
            ...
            switch(line[(*addr_i)++]){
                ...
                case STRING:
                    ...
                    //printf("get length: %lld\n", length);
                    pkrv.package_ty.pack_f64i8s = DataToString(line, length, *addr_i);
                    pkrv.pack_ty = PACK_DOUBLE_BYTES;
                    //printf("push logo logo\n");
                    pushList(forEach_loop2, __String__, pkrv, length);
                    //printf("empurrou\n");
                    (*addr_i) += (length << 1) - 1;
                    free(pkrv.package_ty.pack_f64i8s);
                    //printf("liberou pkrv.str\n");
                    break;                    
            ...
            if(size_Obj){
                if(line[*addr_i - 1] != BYTE && line[*addr_i - 1] >= LONG && line[*addr_i - 1] <= CHAR){
                ...
                if(qttLoop == 1 && lastLoop == 2){
                    if(ref_ty == __i64__ || ref_ty == __i32__ || ref_ty == __i16__ ||
                    ref_ty == __Char__ || ref_ty == __i8__)
                    {
                        pkrv.package_ty.pack_integer = value;
                        pkrv.pack_ty = PACK_INTEGER;
                        pushList(forEach_loop2, ref_ty, pkrv, strsize(inttstr(value)) + 1);
                    }
                }
...
inline static STATUS COMMAND_START(u8 *line, __System__ *System){
    ...
    initializeBuffer(System->Manager.objectsUsing, 0, OBJECTsQTTY);
...
inline static STATUS COMMAND_SYSIN(u8 *line, __System__ *System){
    ...
    switch(casting){
        ...
        case INT:
            ...
            (i32)convert_StringToi32eger(input);
            break;
        case SHORT:
    ...
    //printf("Antes de liberar input\n");
    free(input);
    //printf("Apos liberar input\n");
    free(objects);
    free(address);
    //printf("Antes de liberar values\n");
    freeList(values);
    //printf("Depois de liberar values\n");
    ...
    while(command != END){
        switch(command){
            ...
            case SYSIN:
                ...
                //printf("int val -> %ld\n", System->Manager.Objects.st_i32[0]->i32Ty);
                addr_l++;
                break;
